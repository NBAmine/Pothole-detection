...
compiled = compile_model("app/models/model_openvino/model.xml", "CPU")
...
def infer_openvino(img, conf_threshold: float = 0.25, iou_threshold: float = 0.45):
    
    # Preprocess
    img = cv2.resize(img, (640, 640))

    # OpenVino expects shape [Channels, Height, Width]
    ...
    result = infer_request.infer({input_name: img})
